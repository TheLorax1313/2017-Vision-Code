...
public class Robot extends IterativeRobot {
	...
	public void autonomousPeriodic() {
		...
		SmartDashboard.putNumber("Center Point X", XFinal);
		SmartDashboard.putNumber("Center Point Y", YFinal);
		SmartDashboard.putNumber("***XFINAL***",XFinal);
		switch (autoSelected) {
		case 0:	// RiverTODO: Rename to correct name
			default:
				if((avgDistance < 75)){
				Auto.drive(0.25,XFinal);
				}else{
					fl.set(0);
					bl.set(0);
					fr.set(0);
	...
	public void teleopPeriodic() {
		...
		Relay.Value light=Relay.Value.kOn;
		...
		// RiverTODO: Use Enum for ControllerChooser values in JoyToggle cases (see: https://docs.oracle.com/javase/tutorial/java/javaOO/enum.html )
	        case 0://Dual Joystick tank
				...
				LidModeSwitch = Joy1.getRawButton(2);
				Climb.set((-(Joy1.getRawAxis(3))+1)/2);
				POV.POVJoystick();
				POVAxis = POV.POVFinal();
	        	break;
