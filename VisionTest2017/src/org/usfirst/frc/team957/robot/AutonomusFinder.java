package org.usfirst.frc.team957.robot;

import com.ctre.CANTalon;

import edu.wpi.first.wpilibj.IterativeRobot;
import edu.wpi.first.wpilibj.RobotDrive;
import edu.wpi.first.wpilibj.RobotDrive.MotorType;
import edu.wpi.first.wpilibj.networktables.NetworkTable;
import edu.wpi.first.wpilibj.smartdashboard.SendableChooser;
import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;

@SuppressWarnings("unused")
public class AutonomusFinder {
	double acceptedXFinal = -666;
	double acceptedYFinal = -666;
	NetworkTable PiVision;
	double contourCounter = 0;
	double acceptedContour1;
	double acceptedContour2;
	CANTalon fl = new CANTalon(2);
	CANTalon bl = new CANTalon(3);
	CANTalon fr = new CANTalon(4);
	CANTalon br = new CANTalon(5);
	double speedAdjust = 0.25;
	double speedChangeDivisor = 4;
	double distanceConstant = 80;
	double minAR = 2.2;
	double maxAR = 2.8;
	double AV = 5; //Adjustment Value to detect contours
	RobotDrive m_Drive = new RobotDrive(fl, bl, fr, br);
	
	
	public void talonInit(){
		m_Drive.setInvertedMotor(MotorType.kFrontRight, true);
	    m_Drive.setInvertedMotor(MotorType.kRearRight, true);	
	}
	
	
	public void AutoDetect(){
		NetworkTable Pi_RioCom = NetworkTable.getTable("datatable");
		acceptedXFinal = -666;
		acceptedYFinal = -666;
		
		boolean contour0Poss = true;
		boolean contour1Poss = true;
		boolean contour2Poss = true;
		boolean contour3Poss = true;
		contourCounter = 0;
		acceptedContour1 = -1;
		acceptedContour2 = -1;
		boolean contoursCorrect = false;
		
		double x0 = Pi_RioCom.getNumber("X0",0);
		double x1 = Pi_RioCom.getNumber("X1",2);
		double x2 = Pi_RioCom.getNumber("X2",4);
		double x3 = Pi_RioCom.getNumber("X3",6);
		
		double y0 = Pi_RioCom.getNumber("Y0",1);
		double y1 = Pi_RioCom.getNumber("Y1",3);
		double y2 = Pi_RioCom.getNumber("Y2",5);
		double y3 = Pi_RioCom.getNumber("Y3",7);
		
		double width0 = Pi_RioCom.getNumber("X4",8);
		double width1 = Pi_RioCom.getNumber("X5",9);
		double width2 = Pi_RioCom.getNumber("X6",10);
		double width3 = Pi_RioCom.getNumber("X7",11);
		
		double height0 = Pi_RioCom.getNumber("Y4",13);
		double height1 = Pi_RioCom.getNumber("Y5",14);
		double height2 = Pi_RioCom.getNumber("Y6",15);
		double height3 = Pi_RioCom.getNumber("Y7",16);
		
		SmartDashboard.putNumber("x0",x0);
		SmartDashboard.putNumber("x1",x1);
		SmartDashboard.putNumber("x2",x2);
		SmartDashboard.putNumber("x3",x3);
		
		SmartDashboard.putNumber("y0",y0);
		SmartDashboard.putNumber("y1",y1);
		SmartDashboard.putNumber("y2",y2);
		SmartDashboard.putNumber("y3",y3);
		
		SmartDashboard.putNumber("width0",width0);
		SmartDashboard.putNumber("width1",width1);
		SmartDashboard.putNumber("width2",width2);
		SmartDashboard.putNumber("width3",width3);
		
		SmartDashboard.putNumber("height0",height0);
		SmartDashboard.putNumber("height1",height1);
		SmartDashboard.putNumber("height2",height2);
		SmartDashboard.putNumber("height3",height3);

		double acceptedWidth1 = -1;
		double acceptedWidth2 = -1;
		double acceptedHeight1 = -1;
		double acceptedHeight2 = -1;
		double acceptedX1 = -1;
		double acceptedX2 = -1;
		double acceptedY1 = -1;
		double acceptedY2 = -1;
		
		double CameraResX = 270;
		double CameraResY= 370;

		contourAccepted(x0,y0,height0,width0,0);
		contourAccepted(x1,y1,height1,width1,1);
		contourAccepted(x2,y2,height2,width2,2);
		contourAccepted(x3,y3,height3,width3,3);
		
		SmartDashboard.putNumber("contour1", acceptedContour1);
		SmartDashboard.putNumber("contour1", acceptedContour2);
		
		
		if(contourCounter == 2){
			
			contoursCorrect = true;
			
			if(acceptedContour1 == 0){
				acceptedWidth1 = width0;
				acceptedHeight1 = height0;
				acceptedX1 = x0;
				acceptedY1 = y0;
			}
			if(acceptedContour1 == 1){
				acceptedWidth1 = width1;
				acceptedHeight1 = height1;
				acceptedX1 = x1;
				acceptedY1 = y1;
			}
			if(acceptedContour1 == 2){
				acceptedWidth1 = width2;
				acceptedHeight1 = height2;
				acceptedX1 = x2;
				acceptedY1 = y2;
			}
			if(acceptedContour1 == 3){
				acceptedWidth1 = width3;
				acceptedHeight1 = height3;
				acceptedX1 = x3;
				acceptedY1 = y3;
			}
			if(acceptedContour2 == 0){
				acceptedWidth2 = width0;
				acceptedHeight2 = height0;
				acceptedX2 = x0;
				acceptedY2 = y0;
			}
			if(acceptedContour2 == 1){
				acceptedWidth2 = width1;
				acceptedHeight2 = height1;
				acceptedX2 = x1;
				acceptedY2 = y1;
			}
			if(acceptedContour2 == 2){
				acceptedWidth2 = width2;
				acceptedHeight2 = height2;
				acceptedX2 = x2;
				acceptedY2 = y2;
			}
			if(acceptedContour2 == 3){
				acceptedWidth2 = width3;
				acceptedHeight2 = height3;
				acceptedX2 = x3;
				acceptedY2 = y3;
			}
			acceptedXFinal = ((acceptedX1 + acceptedX2)/2) - ((CameraResX)/2) ;
			acceptedYFinal = ((acceptedY1 + acceptedY2)/2) - CameraResY;
			SmartDashboard.putNumber("Center Point X", acceptedXFinal);
			SmartDashboard.putNumber("Center Point Y", acceptedYFinal);
			//Now that we have all necessary data, we run the main auto code below here
			
					
		}else{
			
		}
	}
	
		
	public void drive(double speed, double XFinal){
		
		if(!(XFinal == -666)){
			if(XFinal < 5 && XFinal > -5){
				m_Drive.mecanumDrive_Cartesian(0,-speed,0,0);
			}else{
				if(XFinal < 0){
					m_Drive.mecanumDrive_Cartesian((-speed*1.5),0,0,0);
				}else{
					m_Drive.mecanumDrive_Cartesian((speed*1.5),0,0,0);
				}
				
			}
			/*
			double fl_speed = speed + (XFinal*0.0005);
			double bl_speed = speed + (XFinal*0.0005);
			double fr_speed = speed - (XFinal*0.0005);
			double br_speed = speed - (XFinal*0.0005);
			
			fl.set(fl_speed);
			bl.set(bl_speed);
			fr.set(-fr_speed);
			br.set(-br_speed);
			*/
			
		}else{
			/*
			fl.set(speed);
			bl.set(speed);
			fr.set(-speed);
			br.set(-speed);	
			*/
			m_Drive.mecanumDrive_Cartesian(0,-speed,0,0);
			
		}
	}
	public void contourAccepted(double x, double y, double h, double w, double cont){
		if(h/w > minAR && h/w < maxAR){
			contourCounter++;
			if(acceptedContour1 == -1){
				acceptedContour1 = cont;
			}else{
			if(acceptedContour2 == -1){
				acceptedContour2 = cont;
			}
		}
	}
	}
	
	public double acceptedXFinal() {
		return acceptedXFinal;
	}
	public double acceptedYFinal() {
		return acceptedYFinal;
	}

}
